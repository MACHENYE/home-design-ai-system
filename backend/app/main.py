...
TASK_CANCEL_MESSAGE = "用户已终止生成任务"
TASK_TIMEOUT_MESSAGE = "任务生成超时，已自动结束"
PROTECTED_TERMINAL_MESSAGES = {TASK_CANCEL_MESSAGE, TASK_TIMEOUT_MESSAGE}


def _protected_terminal_task(rec: TaskRecord | None) -> bool:  # 判断任务是否为本地终止或超时，避免被远程回调覆盖
    return bool(
        rec
        and rec.status == NanoBananaTaskStatus.failed
        and (rec.error_message or "") in PROTECTED_TERMINAL_MESSAGES
    )


def _expire_stale_generation_tasks(user_id: int | None = None, task_id: str | None = None) -> int:  # 按配置超时时间自动结束长时间运行的任务
    return store.expire_stale_tasks(
        settings.generation_task_timeout_s,
        user_id=user_id,
        task_id=task_id,
...
async def _process_queued_generation(task_id: str) -> None:  # 从数据库读取任务参数并执行一次实际图像生成
    started = time.perf_counter()
    _expire_stale_generation_tasks(task_id=task_id)
    ...
    if _protected_terminal_task(latest) or (latest and latest.status == NanoBananaTaskStatus.failed):
        return
...
def _mark_queued_generation_failed(task_id: str, message: str, raw: dict[str, object]) -> None:  # 在队列任务失败时统一更新任务和设计记录状态
    if _protected_terminal_task(store.get(task_id)):
        return
    store.update_result(
        task_id,
        status=NanoBananaTaskStatus.failed,
...
async def list_tasks(limit: int = 50, user: UserProfile = Depends(current_user)) -> list[TaskRecord]:  # 返回当前用户最近提交的生成任务列表
    _expire_stale_generation_tasks(user_id=user.id)
...
async def refresh_task(task_id: str) -> TaskRecord:  # 主动查询远程任务状态并同步更新本地记录
    ...
    if not rec:
        raise HTTPException(status_code=404, detail="task not found")
    if _protected_terminal_task(rec):
        return rec
    ...
    latest = store.get(task_id)
    if _protected_terminal_task(latest):
        return latest
...
async def cancel_task(task_id: str, user: UserProfile = Depends(current_user)) -> dict[str, bool]:  # 当前用户主动终止并删除自己的生成任务
    deleted = store.cancel_and_delete_task(task_id, user.id)
    if not deleted:
        raise HTTPException(status_code=404, detail="task not found")
    _invalidate_data_cache()
    _log_event("task_cancelled", user=user, target_type="task", target_id=task_id, message=TASK_CANCEL_MESSAGE)
    return {"deleted": True}
...
async def nanobanana_callback(cb: dict[str, object]) -> dict[str, str]:  # 接收 NanoBanana 异步回调并更新任务结果
    ...
    if store.is_cancelled_task_identifier(str(task_id)):
        return {"ok": "true"}
    ...
    if store.is_cancelled_task_identifier(remote_task_id) or store.is_cancelled_task_identifier(target_task_id):
        return {"ok": "true"}

    existing = store.get(target_task_id)
    if _protected_terminal_task(existing):
        return {"ok": "true"}
